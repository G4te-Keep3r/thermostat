...
//get base data
$query = sprintf("CREATE TEMPORARY TABLE tmp SELECT recordID, temp, highCutOff, lowCutOff, temp_hall, outside, attic, state FROM temp_graphs order by recordID DESC LIMIT 1440");
...
//order by
$query = sprintf("SELECT recordID, temp, highCutOff, lowCutOff, temp_hall, outside, attic, state FROM tmp ORDER BY tmp.recordID");
$result = $mysqli->query($query);
